...
import { getTaskSourceProviderIdentityLabel } from './task-source-provider-identity-label'
...
function getRepoBackedTaskSourceSummary(args: {
  ...
  const identityLabels = uniqueLabels(
    contexts.map((context) => getTaskSourceProviderIdentityLabel(context.providerIdentity))
  )
